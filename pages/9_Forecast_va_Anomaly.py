...
if not HAS_STATSMODELS:
    st.warning(
        "⚠️ Thiếu thư viện `statsmodels`. Thêm vào `requirements.txt` và deploy lại. "
...
if HAS_STATSMODELS and len(series) >= 28:
    try:
        ...
        with col_r:
            st.markdown("**4️⃣ Residual** — biến động ngẫu nhiên")
...
if len(series) >= 14:
    ...
    if not pd.isna(current_cv):
        if current_cv < 30:
            ...
            st.warning(
                f"⚠️ Doanh thu **BIẾN ĐỘNG CAO** (CoV = {current_cv:.1f}% > 60%). "
